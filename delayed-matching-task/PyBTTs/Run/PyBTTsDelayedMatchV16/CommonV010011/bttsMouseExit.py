...
class BTTsMouseExit():
    ...
    def containPos(_cPos, _cSize, _pos):
        flagX = BTTsMouseExit.areaIn(_cPos[0], _cSize[0], _pos[0]);
        flagY = BTTsMouseExit.areaIn(_cPos[1], _cSize[1], _pos[1]);
    ...
    def winflip(self):
        ...
        if self.seqBtn == 0:
            if buttons[2]:
                curFlag = BTTsMouseExit.containPos(curArea,self.areaSize,[x,y])
                pass;
                if curFlag:
                    self.seqBtn = 1;
                    pass;
